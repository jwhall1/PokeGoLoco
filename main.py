...
def main():
    ...
    parser.add_argument("-m", "--mail", help="Phone Number@carrier", required=True)
    ...
    if api_endpoint is None:
        f = open("Output.txt", "w")
        f.write("Server offline, try again later")
        f.close()
    ...
    while True:
        ...
        for poke in visible:
            ...
            direction = (('N' if difflat >= 0 else 'S') if abs(difflat) > 1e-4 else '')  + (('E' if difflng >= 0 else 'W') if abs(difflng) > 1e-4 else '')
            f = open("Output.txt", "a")
            f.write("%s %ss (%sm %s)\n" % (pokemons[poke.pokemon.PokemonId - 1]['Name'],poke.TimeTillHiddenMs / 1000, int(origin.get_distance(other).radians * 6366468.241830914), direction))
            f.close()
        ...
        print('')
        s = "python SendText.py "+ args.mail
        print(s)
        os.system(s)
        walk = getNeighbors()
